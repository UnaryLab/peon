"""Per-thread isolated read-write workdir (shared by both backends + uploads).

When a thread turns write-mode ON, the CLI's edits are confined to a dedicated
directory under WORKDIR_BASE (env, default ~/Projects/.peon-workdirs), namespaced
by agent + thread_ts. This is the single owner of that path scheme; app.py (for
outbound file uploads) and both runners (cwd + claude --add-dir) reuse it. The
base is read LIVE from os.environ so a SIGHUP .env reload takes effect. The
default base is ABSOLUTE (~/Projects/.peon-workdirs, OUTSIDE this repo on
purpose), so a write-mode agent's edits never touch the framework source;
get_workdir always returns an ABSOLUTE path (the subprocess cwd + claude
--add-dir need one). Set an explicit WORKDIR_BASE to override the default.
"""
...
def get_workdir(agent_name, thread_ts, create=False):
    """The isolated read-write workdir path for (agent_name, thread_ts).
    ...
    makes the directory on demand (used when write-mode actually runs).
    ...
    required because the subprocess cwd and claude --add-dir cannot take a
    relative/ambiguous path. With the default base this resolves to
    <home>/Projects/.peon-workdirs/<agent>/<thread>, i.e. OUTSIDE the repo.

    Shared helper: both runners use it (subprocess cwd + claude --add-dir) and
    app.py reuses it for the outbound file-upload scan.
    """
